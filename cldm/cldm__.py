...
class ControlledUnetModel(UNetModel):
    def forward(self, x, timesteps=None, context=None, control=None, only_mid_control=False, c_mask=None, **kwargs):
        ...
        with torch.no_grad():
            ...
            kernel = torch.ones((num_channels, 1, 5, 5))
...
class ControlLDM(LatentDiffusion):
    ...
    def apply_model(self, x_noisy, t, cond, *args, **kwargs):
        ...
        # print(np.shape(cond['c_crossattn'][0]['c0'])) # train [1, 257, 1024, 2] (batch=1) -> [2, 257, 1024, 2] (batch=2)
        #                                               # test [[2, 257, 1024, 2] (batch=1) -> [4, 257, 1024, 2] (batch=2)
        # print(np.shape(cond['c_crossattn'][0]['c1']))
        # print(np.shape(cond['c_crossattn'][0]['c2']))
        # print(np.shape(cond['c_crossattn'][0]['c3']))
        # print(np.shape(cond['c_crossattn'][0]['c4']))
        # print(np.shape(cond['c_crossattn'][0]['c5']))
        # import pdb; pdb.set_trace()

        # cond_txt = torch.cat(cond['c_crossattn'], 1)
        cond_txt0 = cond['c_crossattn'][0]['c0']
        cond_txt1 = cond['c_crossattn'][0]['c1']
        cond_txt2 = cond['c_crossattn'][0]['c2']
        cond_txt3 = cond['c_crossattn'][0]['c3']
        cond_txt4 = cond['c_crossattn'][0]['c4']
        cond_txt5 = cond['c_crossattn'][0]['c5']

        # print('cond_txt0 shape: ', np.shape(cond_txt0)) # [12, 257, 1024, 2]

        # cond_txt0 = cond_txt0[0] if isinstance(cond_txt0, list) else cond_txt0
        # cond_txt1 = cond_txt1[0] if isinstance(cond_txt1, list) else cond_txt1
        # cond_txt2 = cond_txt2[0] if isinstance(cond_txt2, list) else cond_txt2
        # cond_txt3 = cond_txt3[0] if isinstance(cond_txt3, list) else cond_txt3
        # cond_txt4 = cond_txt4[0] if isinstance(cond_txt4, list) else cond_txt4
        # cond_txt5 = cond_txt5[0] if isinstance(cond_txt5, list) else cond_txt5

        # print(np.shape(cond_txt0)) # [2, 257, 1024, 2] (batch=1) -> [4, 257, 1024, 2] (batch=2) [test]
        # import pdb; pdb.set_trace()

        cond_txt = torch.stack([cond_txt0, cond_txt1, cond_txt2, cond_txt3, cond_txt4, cond_txt5], dim=3)


        # print('c_crossattn shape', np.shape(cond_txt)) # [4, 257, 1024, 2] (batch=2) -> [4, 257, 1024, 6, 2] (batch=2) [test]
        # wwwww
        hint = torch.cat(cond['c_concat'], 1)
        # print('now!, c_concat shape', np.shape(hint)) # [B, 4, 512, 512]
        # print('now!, cond_txt shape', np.shape(cond_txt)) # [1, 257, 1024, 2] -> [12, 257, 1024, 6, 2]
        # print(np.shape(hint)) # [1, 4, 512, 512] -> [2, 4, 512, 512] (batch=2)
    ...
    def log_images(self, batch, N=4, n_row=2, sample=False, ddim_steps=50, ddim_eta=0.0, return_keys=None,
        ...
        cond_image = batch[self.cond_stage_key+'0'+'0'].cpu().numpy().copy() ####### need adjustment
