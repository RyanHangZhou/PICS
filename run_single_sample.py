...
def inference_single_image(ref_image, ref_mask, tar_image, tar_mask, guidance_scale = 5.0):
    ...
    control = einops.rearrange(control, 'b h w c -> b c h w').clone() # [512, 512, 4] object patch
    # import pdb; pdb.set_trace()


    clip_input = torch.from_numpy(ref.copy()).float().cuda() 
    clip_input = torch.stack([clip_input for _ in range(num_samples)], dim=0)
    clip_input = einops.rearrange(clip_input, 'b h w c -> b c h w').clone() # [224, 224, 3] condition
