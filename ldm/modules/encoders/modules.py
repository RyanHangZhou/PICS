...
config_path = './configs/grabcompose.yaml'
config = OmegaConf.load(config_path)
DINOv2_weight_path = config.model.params.cond_stage_config.weight


# sys.path.append("/home/hang18/links/projects/rrg-vislearn/hang18/COIN/dinov3")
# from omegaconf import OmegaConf
# config_path = './configs/grabcompose.yaml'
# config = OmegaConf.load(config_path)
# from dinov3 import hubconf
# DINOv3_weight_path = config.model.params.cond_stage_config.weight

# 1) 指向 dinov3 仓库“根目录”，该目录下应有 hubconf.py
# from omegaconf import OmegaConf
# repo_root = "/home/hang18/links/projects/rrg-vislearn/hang18/COIN/dinov3"
# sys.path.insert(0, repo_root)           # 放在最前，确保先找到本地 hubconf.py

# # 2) 直接导入根目录的 hubconf.py（而不是 from dinov3 import hubconf）
# import hubconf as d3_hub
...
class FrozenDinoV3Encoder(AbstractEncoder):
    """
    Uses the DINOv3 encoder for image
    """
    def __init__(self, device="cuda", freeze=True):
        super().__init__()
        dinov3 = d3_hub.dinov3_vith16plus(pretrained=False) 
        state_dict = torch.load(DINOv3_weight_path)
        dinov3.load_state_dict(state_dict, strict=False)
        self.model = dinov3.to(device)
        self.device = device
        if freeze:
            self.freeze()
        self.image_mean = torch.tensor([0.485, 0.456, 0.406]).unsqueeze(0).unsqueeze(-1).unsqueeze(-1)
